...
export default async function AdminReviewsPage() {
  ...
  const reviews = reviewsData as ReviewWithBusiness[] | null;
  const businesses = businessesData as Pick<Business, "id" | "name">[] | null;
  ...
  if (businessesError) {
    throw new Error(
      `Failed to load businesses: ${businessesError.message}`
    );
  }
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink/60">
        Reviews are curated/admin-managed — no public accounts are required to
        submit them.
      </p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-line bg-white p-5">
          ...
          <form action={upsertReview} className="mt-4 flex flex-col gap-3">
            ...
            <input
              name="tags"
              placeholder="Tags, comma-separated (good-wifi, quiet)"
              className="rounded-lg border border-line px-3 py-2 text-sm"
            />
            ...
            <button
              type="submit"
              className="rounded-lg bg-ink py-2.5 text-sm font-semibold text-paper hover:bg-ink/90"
            >
              Add review
            </button>
          </form>
              ...
              <li key={r.id} className="py-3 text-sm">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    {r.is_seed_content && (
                      <span className="rounded-full bg-line px-2 py-0.5 text-[10px] text-ink/50">
                        Seed
                      </span>
                    )}
