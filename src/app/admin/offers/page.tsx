import type { Business, Offer } from "@/types/database";
import { createClient } from "@/lib/supabase/server";
import { upsertOffer, deleteOffer } from "@/lib/actions/admin";

export const dynamic = "force-dynamic";

type OfferWithBusiness = Offer & {
  business: {
    name: string;
  } | null;
};
...
export default async function AdminOffersPage() {
  ...
  const offers = offersData as OfferWithBusiness[] | null;
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-line bg-white p-5">
          ...
          <form action={upsertOffer} className="mt-4 flex flex-col gap-3">
            ...
            <input
              name="title"
              placeholder="20% off with student ID"
              required
              className="rounded-lg border border-line px-3 py-2 text-sm"
            />
